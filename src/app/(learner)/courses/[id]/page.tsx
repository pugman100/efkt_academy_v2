  ...
  return (
    <>
      <section style={{ position: 'relative', height: 440, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(12,14,57,0.55) 0%, rgba(12,14,57,0.20) 40%, rgba(12,14,57,0.85) 100%)' }} />
        <div className="lr-x" style={{ position: 'relative', height: '100%', paddingTop: 32, paddingBottom: 48, display: 'flex', flexDirection: 'column', maxWidth: 900, boxSizing: 'border-box' }}>
      ...
      <section className="lr-x" style={{ paddingTop: 40, maxWidth: 960, boxSizing: 'border-box', width: '100%' }}>
        {c.reference ? (
          <div style={{ padding: '24px 28px', background: 'var(--efkt-offwhite)', borderRadius: 20, fontSize: 16, fontWeight: 300, color: 'var(--text-muted)', textWrap: 'pretty' }}>
          ...
          <div style={{ padding: '24px 28px', background: 'var(--efkt-offwhite)', borderRadius: 20 }}>
            ...
            <div style={{ height: 8, borderRadius: 4, background: 'var(--efkt-border)', overflow: 'hidden' }}>
              <div style={{ height: 8, borderRadius: 4, width: progress.pct + '%', background: progress.complete ? 'var(--efkt-green)' : 'var(--efkt-coral)' }} />
            </div>
      ...
      <section className="lr-x" style={{ paddingTop: 32, paddingBottom: 96, maxWidth: 960, boxSizing: 'border-box', width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {progress.items.map((it) => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap', padding: '20px 24px', border: '1px solid var(--border-default)', borderRadius: 20, background: 'var(--surface-card)' }}>
              <span style={{ width: 56, height: 56, minWidth: 56, borderRadius: 12, background: 'var(--efkt-offwhite)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                <i className={`ph ph-${c.reference ? 'file-text' : passed ? 'check-circle' : locked ? 'lock-simple' : 'play-circle'}`} style={{ fontSize: 24, color: 'var(--efkt-coral)' }} aria-hidden />
              </span>
              <div style={{ minWidth: 180, flex: 1 }}>
                <div style={{ fontSize: 16, fontWeight: 600 }}>{it.index + 1}. {m.title}</div>
                <div style={{ fontSize: 14, fontWeight: 300, color: 'var(--text-muted)', marginTop: 4, overflowWrap: 'anywhere' }}>
                  {c.reference
                    ? m.source === 'BUILT' ? 'Bygget her' : m.url
                    : m.quiz
              ...
              {showQuiz ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-start' }}>
                  {left === 0 && !passed ? (
                    <ButtonLink href={moduleHref(m.id)} prefetch={false} iconRight="arrow-counter-clockwise">Gå gjennom modulen på nytt</ButtonLink>
                  ) : (
                    <ButtonLink href={`${moduleHref(m.id)}/quiz`} prefetch={false} iconRight="list-checks">
                      {passed ? 'Ta quizen igjen' : failed ? 'Prøv igjen' : 'Ta quizen'}
                    </ButtonLink>
                  )}
                  <span style={{ fontSize: 14, fontWeight: 300, color: 'var(--text-muted)' }}>
              ...
              {locked ? (
                <span style={{ fontSize: 14, fontWeight: 300, color: 'var(--text-muted)', maxWidth: 220, textWrap: 'pretty' }}>Åpnes når forrige modul er bestått</span>
              ) : null}
              {gateWaiting ? (
